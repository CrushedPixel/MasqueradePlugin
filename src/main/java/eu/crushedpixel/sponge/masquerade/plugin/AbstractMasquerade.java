...
import eu.crushedpixel.sponge.masquerade.plugin.data.EntityMetadata;
...
import static eu.crushedpixel.sponge.masquerade.plugin.utils.PacketUtils.velocityToShort;
...
public abstract class AbstractMasquerade implements Masquerade {
    ...
    @Override
    public void setValueChangeAllowed(Key<?> key, boolean valueChangeAllowed) {
        Preconditions.checkArgument(hasKey(key), "Unknown key for this masquerade type");
        EntityMetadata<?, ?> entityMetadata = metadata.get(key);
        entityMetadata.setValueChangeAllowed(valueChangeAllowed);
    }

    @Override
    public boolean isValueChangeAllowed(Key<?> key) {
        Preconditions.checkArgument(hasKey(key), "Unknown key for this masquerade type");
        EntityMetadata<?, ?> entityMetadata = metadata.get(key);
        return entityMetadata.isValueChangeAllowed();
    }
